...
// config.js → window.APP_FIREBASE_CONFIG এ সব credential আছে
...
/* ─── Theme toggle ──────────────────────────────────────────── */
function initTheme() {
    const saved = localStorage.getItem(window.APP_IDENTITY.theme_key);
    if (saved === 'light') document.body.classList.add('theme-light');
}
window.toggleTheme = () => {
    document.body.classList.toggle('theme-light');
    localStorage.setItem(window.APP_IDENTITY.theme_key, document.body.classList.contains('theme-light') ? 'light' : 'dark');
};
...
function loadData() {
    ...
    onSnapshot(collection(db, DB_PATH), async snap => {
        allFiles = [];
        snap.forEach(child => {
            allFiles.push({ id: child.id, ...child.data() });
        });
...
function loadFolders() {
    if (!navigator.onLine) return;
    onSnapshot(collection(db, FOLDERS_PATH), async snap => {
        folders = [];
        snap.forEach(child => folders.push({ id: child.id, ...child.data() }));
...
function loadSettings() {
    if (!navigator.onLine) return;
    onSnapshot(doc(db, SETTINGS_PATH, 'main'), snap => {
        const s = snap.data();
        if (!s) return;
...
async function processSyncQueue() {
    ...
    for (const item of items) {
        try {
            if (item.type === 'update') await updateDoc(doc(db, DB_PATH, item.id), item.data);
            if (item.type === 'delete') await deleteDoc(doc(db, DB_PATH, item.id));
            if (item.type === 'create') await setDoc(doc(db, DB_PATH, item.id), item.data);
            if (item.type === 'folderCreate') await setDoc(doc(db, FOLDERS_PATH, item.id), item.data);
            if (item.type === 'folderDelete') await deleteDoc(doc(db, FOLDERS_PATH, item.id));
...
async function uploadQueuedItem(file, customName, folder) {
    ...
    fd.append('file', file);
    // Cloudinary config → config.js থেকে নেওয়া হচ্ছে
    fd.append('upload_preset', window.APP_CLOUDINARY.upload_preset);
    const res  = await fetch(`https://api.cloudinary.com/v1_1/${window.APP_CLOUDINARY.cloud_name}/auto/upload`, { method: 'POST', body: fd });
    ...
    const newRef = await addDoc(collection(db, DB_PATH), rec);
}
...
window.openNexusLightbox = fileId => {
    ...
    NexusLightbox.open(lbItems, startIdx >= 0 ? startIdx : 0, {
        onStar: async (id, curStarred) => {
            ...
            render();
            if (navigator.onLine) updateDoc(doc(db, DB_PATH, id), { starred: f.starred });
...
function uploadSingleFile(file, customName, folder, onProgress) {
    return new Promise((resolve, reject) => {
        ...
        xhr.open('POST', `https://api.cloudinary.com/v1_1/${window.APP_CLOUDINARY.cloud_name}/auto/upload`);
        xhr.send(fd);
    });
}
...
async function fbUpdate(id, data) {
    ...
    if (navigator.onLine) {
        updateDoc(doc(db, DB_PATH, id), data);
    } else {
    ...
    showModal({
        ...
        btns:  [
            ...
            { label: 'Delete Forever', cls: 'modal-btn-danger', action: async () => {
                closeModal();
                if (navigator.onLine) { await deleteDoc(doc(db, DB_PATH, id)); }
        ...
        btns:   [
            ...
            { label: move ? 'Move' : 'Copy', cls: 'modal-btn-confirm', action: async () => {
                ...
                if (move) {
                    ...
                    data.time   = Date.now();
                    const newRef = doc(collection(db, DB_PATH));
                    if (navigator.onLine) await setDoc(newRef, data);
                    else await addToSyncQueue({ type: 'create', id: newRef.id, data });
        ...
        btns:  [
            ...
            { label: 'Empty Trash', cls: 'modal-btn-danger', action: async () => {
                ...
                for (const f of trashed) {
                    if (navigator.onLine) await deleteDoc(doc(db, DB_PATH, f.id));
    ...
    showModal({
        ...
        btns: [
            ...
            { label: 'Create', cls: 'modal-btn-confirm', action: async () => {
                ...
                closeModal();
                const newRef = doc(collection(db, FOLDERS_PATH));
                const fData = { name, color, time: Date.now() };
                if (navigator.onLine) await setDoc(newRef, fData);
                else await addToSyncQueue({ type:'folderCreate', id: newRef.id, data: fData });
                ...
                showModal({
                    ...
                    btns: [
                        ...
                        { label: 'Rename', cls: 'modal-btn-confirm', action: async () => {
                            ...
                            if (!n) return;
                            closeModal();
                            if (navigator.onLine) await updateDoc(doc(db, FOLDERS_PATH, fid), { name: n });
                ...
                showModal({
                    ...
                    btns:  [
                        ...
                        { label: 'Delete', cls: 'modal-btn-danger', action: async () => {
                            closeModal();
                            if (navigator.onLine) await deleteDoc(doc(db, FOLDERS_PATH, fid));
    ...
    showModal({
        ...
        btns:  [
            ...
            { label: 'Update', cls: 'modal-btn-confirm', action: async () => {
                ...
                closeModal();
                appPasscode = n;
                if (navigator.onLine) setDoc(doc(db, SETTINGS_PATH, 'main'), { passcode: n }, { merge: true });
