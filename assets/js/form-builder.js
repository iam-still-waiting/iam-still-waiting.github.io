...
(function () {
    ...
    window.showIpPopup = function(index) {
        ...
        modal.innerHTML = `
            <div style="background:#1a1a2e;border:1px solid rgba(124,58,237,0.4);border-radius:16px;padding:1.5rem;max-width:380px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.5);">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.2rem;">
                    <h3 style="margin:0;font-size:1.1rem;background:linear-gradient(135deg,#7c3aed,#06b6d4);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">🌍 IP Information</h3>
                    <button onclick="document.getElementById('ipInfoModal').remove()" style="background:none;border:none;color:rgba(255,255,255,0.5);cursor:pointer;font-size:1.3rem;line-height:1;">&times;</button>
                </div>
                <div style="display:flex;flex-direction:column;gap:0.7rem;">
                    ${[
                        ...
                        ['📍 Coordinates', ip.latitude && ip.longitude ? ip.latitude + ', ' + ip.longitude : null],
                    ].filter(([,v]) => v).map(([label, val]) => `
                        <div style="display:flex;justify-content:space-between;align-items:center;padding:0.5rem 0.75rem;background:rgba(255,255,255,0.05);border-radius:8px;gap:1rem;">
                            <span style="color:rgba(255,255,255,0.5);font-size:0.85rem;white-space:nowrap;">${label}</span>
                            <span style="color:#fff;font-size:0.85rem;font-weight:500;text-align:right;word-break:break-all;">${val}</span>
                        </div>
                    `).join('')}
                </div>
                ${ip.latitude && ip.longitude ? `
                <a href="https://www.google.com/maps?q=${ip.latitude},${ip.longitude}" target="_blank"
                   style="display:flex;align-items:center;justify-content:center;gap:0.5rem;margin-top:1rem;padding:0.6rem;background:rgba(124,58,237,0.2);border:1px solid rgba(124,58,237,0.4);border-radius:8px;color:#7c3aed;text-decoration:none;font-size:0.85rem;font-weight:500;">
                    📌 View on Google Maps
                </a>` : ''}
            </div>
        `;
