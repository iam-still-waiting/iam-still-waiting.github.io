...
async function processPendingUploads() {
    ...
    for (const item of pending) {
        try {
            ...
            form.append('upload_preset', window.APP_CLOUDINARY.upload_preset);
            const res = await fetch(`https://api.cloudinary.com/v1_1/${window.APP_CLOUDINARY.cloud_name}/auto/upload`, { method: 'POST', body: form });
...
import { getFirestore, collection, doc, addDoc, setDoc, getDoc, onSnapshot, deleteDoc, updateDoc } from "https://www.gstatic.com/firebasejs/11.0.1/firebase-firestore.js";
...
// Wire up Firebase helpers (Firestore)
fbUpdate = async (path, data) => {
    const parts = path.split('/');
    await updateDoc(doc(db, parts[0], parts[1]), data);
};
fbRemove = async (path) => {
    const parts = path.split('/');
    await deleteDoc(doc(db, parts[0], parts[1]));
};
fbSet = async (path, data) => {
    // SETTINGS_PATH হলে single doc 'main' এ save হবে
    if (!path.includes('/')) {
        await setDoc(doc(db, path, 'main'), data, { merge: true });
    } else {
        const parts = path.split('/');
        await setDoc(doc(db, parts[0], parts[1]), data, { merge: true });
    }
};
fbPush = async (path, data) => {
    const newRef = await addDoc(collection(db, path), data);
    return newRef.id;
};
fbGet = async (path) => {
    if (!path.includes('/')) {
        const snap = await getDoc(doc(db, path, 'main'));
...
async function loadSettings() {
    ...
    onSnapshot(doc(db, SETTINGS_PATH, 'main'), async snap => {
...
window.startUpload = async () => {
    ...
    function uploadFile(idx) {
        ...
        const form=new FormData(); form.append("file",file); form.append("upload_preset",window.APP_CLOUDINARY.upload_preset);
